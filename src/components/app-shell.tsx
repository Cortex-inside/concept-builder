  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-4 lg:px-6">
        <Link to="/" className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-lg bg-[#102a43] text-white"><Building2 size={20}/></span><span><strong className="block text-[15px] tracking-tight text-[#102a43]">Concept Builder</strong><small className="hidden text-[10px] font-semibold uppercase tracking-[.16em] text-slate-400 sm:block">B2B · Moçambique</small></span></Link>
        <button className="rounded-lg p-2 text-slate-600 md:hidden" onClick={()=>setOpen(v=>!v)} aria-label="Menu">{open?<X/>:<Menu/>}</button>
        <nav className="hidden items-center gap-1 md:flex">{links.map(([to,key])=><Link key={to} to={to} className={location.pathname===to ? "rounded-lg bg-[#e8f5f3] px-3 py-2 text-sm font-semibold text-[#0b5f59]" : "rounded-lg px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-[#102a43]"}>{t(key)}</Link>)}<span className="mx-2 h-5 w-px bg-slate-200"/><button onClick={()=>setLang(lang==="pt"?"en":"pt")} className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600">{lang.toUpperCase()}</button><Link to="/login" className="ml-2 px-3 py-2 text-sm font-semibold text-[#102a43]">{t("nav.signin")}</Link><Link to="/register" className="rounded-lg bg-[#0f766e] px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-[#0b5f59]">{t("nav.register")}</Link></nav>
      </div>
      {open&&<nav className="border-t bg-white px-4 py-4 md:hidden">{links.map(([to,key])=><Link key={to} to={to} onClick={()=>setOpen(false)} className="block border-b border-slate-100 py-3 text-sm font-medium">{t(key)}</Link>)}<div className="mt-4 flex gap-2"><button onClick={()=>setLang(lang==="pt"?"en":"pt")} className="rounded-lg border px-3 py-2 text-sm font-bold">{lang.toUpperCase()}</button><Link to="/login" onClick={()=>setOpen(false)} className="rounded-lg px-3 py-2 text-sm font-semibold">{t("nav.signin")}</Link><Link to="/register" onClick={()=>setOpen(false)} className="rounded-lg bg-[#0f766e] px-3 py-2 text-sm font-semibold text-white">{t("nav.register")}</Link></div></nav>}
